...
public class Connect4AIMinMax {
  ...
  public Connect4AIMinMax(Connect4Board board, int Max_depth, int current_turns) {
...
//g-good, b-bad, z-empty
  ...
  public int getOptimalMove(Connect4Board board, int turns){
      return miniMax(board, turns, MAX_DEPTH, Connect4Board.PLAYER_2_TURN_int)[1];
  }

  // Board, depth max (Vai-se decrementando), jogador atual
  public int[] miniMax(Connect4Board board, int current_turns, int depth_max, int player){
    ...
    if(player==Connect4Board.PLAYER_2_TURN_int){
        ...
        for(int i = 0; i<board.width; i++){
            if(board.canPlace(i)){
                ...
                copiedboard.place(i);
                expandedNodes++;
                int score = miniMax(copiedboard, current_turns+1, depth_max-1, Connect4Board.PLAYER_1_TURN_int)[0];
        ...
        for(int i=0; i<board.width; i++){
            if(board.canPlace(i)){
                ...
                newboard.place(i);
                expandedNodes++;
                int score = miniMax(newboard, current_turns+1, depth_max-1, Connect4Board.PLAYER_2_TURN_int)[0];
                if(score<movesofar[0]){
                    movesofar[0]=score;
                    movesofar[1]=i;
                }
