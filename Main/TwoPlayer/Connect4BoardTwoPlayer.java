...
public class Connect4BoardTwoPlayer {
    ...
    // turn- int for minmax aid
    public static final int PLAYER_1_TURN_int = 1;
    public static final int PLAYER_2_TURN_int = 2;
    ...
    public Connect4BoardTwoPlayer(int width, int height) {
      this.width = width;
      this.height = height;
      board = new int[height][width]; // default all 0
      nextTurn = PLAYER_1_TURN;
    }
    public Connect4BoardTwoPlayer() {
      this(7, 6);
    }
  
    public Connect4BoardTwoPlayer(int[][] contents, boolean nextTurn) {
    ...
    public int getContentsSpecific(int i, int j){
      int contentsCopy=0;
      for(int x = 0; x < height; x++)
        for(int y = 0; y < width; y++)
          contentsCopy = board[x][y];
      return contentsCopy;
    }
    ...
    public String toString() {
      ...
      for(int i = 0; i < height; i++) {
        result += "| ";
        for(int j = 0; j < width; j++) {
          result += (board[i][j] == EMPTY_SLOT ? " " : (board[i][j] == 1 ? "X" : "0"))+" | ";
        }
        result = result.substring(0, result.length()-1);
        result += "\n|-";
        for(int j = 0; j < width; j++) {
